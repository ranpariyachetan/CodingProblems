// You are given n rectangular boxes, the ith box has the length lengthi, the width widthi and the height heighti. Your task is to check if it is possible to
// pack all boxes into one so that inside each box there is no more than one another box (which, in turn, can contain at most one another box, and so on).
// More formally, your task is to check whether there is such sequence of n different numbers pi (1 ≤ pi ≤ n) that for each 1 ≤ i < n 
...
namespace CodingProblems
{
    public class BoxPacking
    {
        ...
        private static bool SolveBoxPacking(int[] length, int[] width, int[] height)
        {
        ...
        public static void TestBoxPacking()
        {
            var widths = new int[] {1, 3, 2};
            var heights = new int[] {1, 3, 2};
            var lengths = new int[] {1, 3, 2};

            var result = SolveBoxPacking(lengths, widths, heights);
